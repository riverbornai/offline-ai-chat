...
export const initializePhi3Model = async (progress?: SetupProgress) => {
  try {
    ...
    progress?.onProgress?.('Initializing Phi-3 model context...');
    await modelStore.initContext(phi3Model);
    progress?.onSuccess?.('Phi-3 model initialized and ready for chat!');
    
    return true;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
    progress?.onError?.(`Error initializing Phi-3 model: ${errorMessage}`);
    console.error('Failed to initialize Phi-3 model:', error);
